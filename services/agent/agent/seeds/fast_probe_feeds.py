...
async def probe_domain(domain: str, client: httpx.AsyncClient) -> list[str]:
    ...
    found_list: list[str] = [url for url in results if url]
    return found_list
...
async def discover_category(category: str, domains: list[str]) -> int:
    ...
    all_feeds_list: list[str] = list(all_feeds)
    print(f"\u2192 Total unique feeds: {len(all_feeds_list)}")
    ...
    if seed_file.exists():
        with open(seed_file) as f:
            ...
            existing_urls: set[str] = set(feed["url"] for feed in existing.get("feeds", []))
    else:
        existing_urls = set()

    # New feeds
    new_feeds = [url for url in all_feeds_list if url not in existing_urls]
    print(f"✅ {len(new_feeds)} NEW | Total will be: {len(new_feeds) + len(existing_urls)}")

    # Merge and save
    feeds_list = [{"url": url, "status": "found_via_probe"} for url in new_feeds]

    if existing_urls:
        feeds_list.extend([{"url": url, "status": "valid"} for url in existing_urls])
...
async def main() -> None:
    ...
    print(f"\n{'=' * 70}")
    print("✨ FAST PROBE DISCOVERY COMPLETE")
